...
現時点では `gh` CLI は利用可能だが、`gh auth status` は token invalid で失敗する。GitHub remote `origin` は `git@github.com:omitsuhashi/openpolitics-lens.git` として存在するが、GitHub Issue mirror は行わない。
