...
- branch: `codex/opl-phase0-remainder-20260707/P0R-009-subsidy-program-candidates-probe`
- base: `7fb7c999aa4f67410379da2fa25a0cf248de2975`
- head: `de1c9bce6d4312995c74e48213134a58949092a8`
- review range: `7fb7c999aa4f67410379da2fa25a0cf248de2975..de1c9bce6d4312995c74e48213134a58949092a8`
- 実装レビュー: 承認済み。独立レビューで Critical / Important なし、Minor 1 件は非 blocking として受容。
...
- branch: `codex/opl-phase0-remainder-20260707/P0R-011-audit-signal-storage-separation`
- base: `ba3fd23e125ed1c502f1487a3b79258fd447b970`
- head: `e793c9e7f3290c742608d1f77bc144b609d4b65c`
- review range: `ba3fd23e125ed1c502f1487a3b79258fd447b970..e793c9e7f3290c742608d1f77bc144b609d4b65c`
- 実装レビュー: 承認済み。独立レビューで Critical / Important / Minor なし。
- 実装内容: `AuditFindingCandidate` と `SpendingReviewSignalCandidate` を別 contract / table / claim type として定義した。`SpendingReviewSignalCandidate` は method version、supporting / counter evidence、limitations、review state、`public_visibility=internal_only` を持つ internal candidate とし、public 表示用 `SpendingReviewSignal`、score、severity を持たない guard を追加した。
- verification: `uv run pytest -q` は 71 passed、`uv run ruff check .` は passed、`uv run ruff format --check .` は passed、`git diff --check` は passed。
- 残リスク: migration / Python contract までで、live database migration apply は未実施。DB 側の evidence item 配列には per-element FK はなく、contract test で確認する scope に留めた。
- blocker release: `P0R-012` を次の実行可能 issue とする。
