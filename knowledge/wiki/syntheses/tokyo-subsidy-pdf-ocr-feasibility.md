...
| 政治資金（暫定 `tokyo_political_funds`） | 未検証。政治資金収支報告書は PDF/OCR pipeline と表抽出の品質検査が必要だが、OCR 実行要否は sample ごとに text layer を検証して判断する。text layer がない sample のみ OCR 必要。 | 東京都選管の report index から RawArtifact 化した PDF を、page 単位の report section と表単位に分ける。政治家本人、後援会、政党支部、資金管理団体を同一 entity にしない。 | 寄附、収入、支出、資産等の表は page / table / row / column を location に持つ。OCR table は review queue に送り、氏名・団体名・金額を自動 merge しない。 | 金額、氏名、団体名は confidence と warning 必須。公式 ID または政治団体名簿 anchor がない場合は RelationshipEdge を作らない。 | `ocr_required`, `ocr_low_confidence`, `table_structure_inferred`, `amount_unit_ambiguous`, `name_or_org_ocr_ambiguous`, `entity_resolution_required`。 | 政治資金 PDF sample set、OCR/text-layer 判定、table extraction evaluator、PoliticalGroup anchor 連携を別 issue 化する。 |
...
| 会議録・議案（暫定 `tokyo_assembly_records_bills`） | 初期対象の都議会 HTML、会議録検索結果、議案ページは OCR 不要。source family 全体では添付 PDF / PDF 本文は未検証。 | 会議録・速記録は meeting / speech 単位、提出議案と議決結果は bill / decision event 単位で HTML parser を作る。個人別賛否が source にない場合は VotePosition を生成しない。 | 議案一覧、議決結果、請願・陳情一覧は HTML table / list parser。会議録本文は table extraction ではなく speech block extraction を使う。 | 公式 HTML selector と speech / bill anchor に戻れる場合は high。検索 UI snapshot 由来は medium。議案上の契約案と電子調達の契約先は同一視しない。 | `search_ui_snapshot`, `table_structure_inferred`, `entity_resolution_required`, `meaning_not_interpreted`。 | 会議録検索 acquisition 条件、議案/bill parser fixture、speech locator、契約案と PublicMoneyFlow を結ばない guard を G2PR-007 後に実装する。 |
