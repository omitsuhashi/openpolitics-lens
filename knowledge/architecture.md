...
ローカル datastore 構成の詳細は [Local Infrastructure](local-infrastructure.md) を正とする。host からの接続は loopback に限定し、アプリケーション container からは Compose network 上の service name で接続する。local runtime の環境変数は `.env.local` に置き、Compose command は `--env-file .env.local` を明示する。
