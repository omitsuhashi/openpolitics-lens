...
def _extract_title(raw_html: bytes) -> _ExtractedTitle:
    ...
    text = _collapse_whitespace(unescape(stripped_title_bytes.decode("utf-8")))
    if not text:
...
def _promote_source_document(record: FetchManifestRecord) -> SourceDocument:
    candidate = record.source_document_candidate
...
def _build_title_evidence_item(
    *,
    ...
    return EvidenceItem(
        ...
        quote_text=extracted_title.text,
        normalized_text=extracted_title.text,
...
def normalize_grant_program_page(
    ...
    source_document = _promote_source_document(record)
